...
describe('ProjectUserController (e2e)', () => {
  ...
  it('/api/v1/projects/:projectId/users (POST) should add all project users if allowed', async () => {
    await request(app.getHttpServer())
    ...
    // check that new user is visible for user 1
    await request(app.getHttpServer())
      .get(`/api/v1/projects/${testProject1.id}/users`)
      .set('Authorization', `Bearer ${testingUser1.accessToken}`)
      .expect(200)
      .expect(res => {
        ...
        const index = res.body.data.reduce(
          (acc, u) => ({
            ...acc,
            [u.userId]: u,
          }),
        ...
        expect(index[testingUser1.id].email).toEqual(testingUser1.email);
        expect(index[testingUser1.id].role).toEqual('admin');

        expect(index[testingUser2.id].email).toEqual(testingUser2.email);
        expect(index[testingUser2.id].role).toEqual('editor');
      });

    // Same for user 2, should now have access to project 1
    await request(app.getHttpServer())
      .get(`/api/v1/projects/${testProject1.id}/users`)
    ...
    // try to get project info from new user
    await request(app.getHttpServer())
      .get(`/api/v1/projects/${testProject1.id}`)
      .set('Authorization', `Bearer ${testingUser2.accessToken}`)
      .expect(200);

    // but should not work the other way around
    await request(app.getHttpServer())
      .get(`/api/v1/projects/${testProject2.id}`)
      .set('Authorization', `Bearer ${testingUser1.accessToken}`)
      .expect(404);
  });

  it('/api/v1/projects/:projectId/users (POST) should reject invites for users that are already project memebers', async () => {
    await request(app.getHttpServer())
      .post(`/api/v1/projects/${testProject1.id}/users`)
      .set('Authorization', `Bearer ${testingUser1.accessToken}`)
      .send({
